...
def count_bigram_relatedness(cleaned_tocken, dictinary_from_tokens):
    list_position = 0
    print(cleaned_tocken)
    for node_value in cleaned_tocken[-1]:
        node1 = cleaned_tocken[list_position]
        node2 = cleaned_tocken[list_position+1]
...
def check_for_relation(node1, node2):
    ...
    print(selected1)
    print(selected2)
    ab = Relationship(selected1, "CO_OCCURENCED", selected2)
    neo4j_transaction.commit()
    #result = neo4j_transaction.commit()
    result = graph.exists(ab)

    #******************* TYPE 4 **********************************#
    res = list(graph.match(selected1,"CO_OCCURENCED",selected2))
    print(res)
